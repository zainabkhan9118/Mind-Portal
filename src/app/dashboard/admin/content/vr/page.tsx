...
import { Plus, Edit2, Trash2, Image as ImageIcon, Search, Eye } from "lucide-react";

const sampleEnvironments = [
  { id: 1, name: "Beach 360°", type: "Nature", image: "https://images.unsplash.com/photo-1506744038136-46273834b3fb?auto=format&fit=facearea&w=400&q=80" },
  { id: 2, name: "Mountain View", type: "Nature", image: "https://images.unsplash.com/photo-1465101046530-73398c7f28ca?auto=format&fit=facearea&w=400&q=80" },
  { id: 3, name: "City Night", type: "Urban", image: "https://images.unsplash.com/photo-1465101178521-c1a9136a3b99?auto=format&fit=facearea&w=400&q=80" },
];

export default function VREnvironmentsPage() {
  const [environments] = useState(sampleEnvironments);
      ...
      <div className="w-full max-w-6xl mx-auto">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-4">
          <h1 className="text-3xl font-extrabold flex items-center gap-2 text-gray-900 dark:text-white">
            <Eye className="w-8 h-8 text-purple-600" /> VR Environments / 360° Images
          </h1>
          <div className="flex flex-1 sm:flex-initial gap-2 items-center">
            ...
            <button className="flex items-center gap-2 bg-gradient-to-r from-purple-500 to-purple-700 text-white px-5 py-2 rounded-lg shadow hover:from-purple-600 hover:to-purple-800 transition font-semibold">
              <Plus className="w-5 h-5" /> Add Environment
            </button>
          </div>
        </div>
          ...
          <table className="w-full text-base">
            <thead>
              <tr className="bg-gray-100 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                ...
                <th className="py-4 px-6 text-left font-bold tracking-wide text-gray-700 dark:text-gray-200">Image</th>
            ...
            <tbody>
              {filteredEnvs.length === 0 ? (
                <tr>
                  <td colSpan={4} className="py-8 px-6 text-center text-gray-400 dark:text-gray-500">No environments found.</td>
                </tr>
              ) : (
                filteredEnvs.map((item) => (
                  <tr key={item.id} className="border-b last:border-b-0 border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 transition">
                    ...
                    <td className="py-3 px-6">
                      <img src={item.image} alt={item.name} className="w-16 h-16 object-cover rounded shadow border border-gray-200 dark:border-gray-700" />
                    </td>
        ...
        {showModal && (
          <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50">
            <div className="bg-white p-6 rounded shadow w-full max-w-md">
              <h2 className="text-lg font-bold mb-4">Add/Edit Environment</h2>
              {/* Form fields here */}
