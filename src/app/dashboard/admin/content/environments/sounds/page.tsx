...
import { Plus, Edit2, Trash2, Volume2, Search, Brain } from "lucide-react";

const sampleSubliminals = [
  { id: 1, name: "Subliminal Focus", type: "Subliminal", icon: <Brain className="w-6 h-6 text-purple-500" />, url: "https://example.com/subliminal-focus.mp3" },
  { id: 2, name: "Deep Calm", type: "Subliminal", icon: <Volume2 className="w-6 h-6 text-blue-500" />, url: "https://example.com/deep-calm.mp3" },
];

export default function SubliminalSoundsPage() {
  const [sounds] = useState(sampleSubliminals);
      ...
      <div className="w-full max-w-4xl mx-auto">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-4">
          <h1 className="text-3xl font-extrabold flex items-center gap-2 text-gray-900 dark:text-white">
            <Volume2 className="w-8 h-8 text-purple-600" /> Subliminal Messages
          </h1>
          <div className="flex flex-1 sm:flex-initial gap-2 items-center">
            ...
            <button className="flex items-center gap-2 bg-gradient-to-r from-purple-500 to-purple-700 text-white px-5 py-2 rounded-lg shadow hover:from-purple-600 hover:to-purple-800 transition font-semibold">
              <Plus className="w-5 h-5" /> Add Subliminal
            </button>
          </div>
        </div>
          ...
          <table className="w-full text-base">
            <thead>
              <tr className="bg-gray-100 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                <th className="py-4 px-6 text-left font-bold tracking-wide text-gray-700 dark:text-gray-200">Icon</th>
            ...
            <tbody>
              {filteredSounds.length === 0 ? (
                <tr>
                  <td colSpan={5} className="py-8 px-6 text-center text-gray-400 dark:text-gray-500">No subliminal messages found.</td>
                </tr>
              ) : (
                filteredSounds.map((item) => (
                  <tr key={item.id} className="border-b last:border-b-0 border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 transition">
                    <td className="py-3 px-6">{item.icon}</td>
        ...
        {showModal && (
          <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50">
            <div className="bg-white p-6 rounded shadow w-full max-w-md">
              <h2 className="text-lg font-bold mb-4">Add/Edit Subliminal</h2>
              {/* Form fields here */}
