...
const ImagesManagement: React.FC = () => {
    ...
    useEffect(() => {
        apiClient
            .get<{ results: GalleryImage[] } | GalleryImage[]>("dashboard/media/images/")
            .then((res) => {
                const data = res.data;
    ...
    const handleUpload = async (file: File) => {
        ...
        try {
            const res = await apiClient.post<GalleryImage>("dashboard/media/images/", fd);
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            await apiClient.delete(`dashboard/media/images/${id}/`);
                        ...
                        {images.map((img) => (
                            <div key={img.id} className="group relative aspect-video rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all">
                                ...
                                <button
                                    ...
                                    className="absolute top-2 right-2 p-1.5 text-white/60 hover:text-white transition-colors disabled:opacity-40"
                                >
